...
const MapView = () => {
  ...
  // Filter out points that fall in the ocean outside India's landmass
  const isWithinIndia = (lat, lon) => {
    // Coarse bounding box for India
    if (lat < 6.5 || lat > 37.5 || lon < 68.0 || lon > 97.5) return false;

    // --- WEST COAST / ARABIAN SEA ---
    if (lon < 72.8 && lat < 22.5) return false;
    if (lon < 73.5 && lat < 18.0) return false;
    if (lon < 74.5 && lat < 14.0) return false;
    if (lon < 76.0 && lat < 10.5) return false;

    // --- EAST COAST / BAY OF BENGAL ---
    // Tight diagonal following the actual eastern coastline
    if (lon > 80.0 && lat < 13.0) return false;
    if (lon > 80.5 && lat < 14.0) return false;
    if (lon > 81.0 && lat < 15.0) return false;
    if (lon > 81.5 && lat < 16.0) return false;
    if (lon > 82.0 && lat < 17.0) return false;
    if (lon > 82.5 && lat < 18.0) return false;
    if (lon > 83.5 && lat < 19.0) return false;
    if (lon > 84.5 && lat < 20.0) return false;
    if (lon > 85.5 && lat < 21.0) return false;
    if (lon > 87.0 && lat < 22.0) return false;
    if (lon > 89.0 && lat < 23.0) return false;
    if (lon > 92.0 && lat < 26.0) return false;

    // --- SRI LANKA / PALK STRAIT ---
    if (lat < 10.0 && lon > 79.0) return false;

    // --- FAR EAST (Myanmar border ocean) ---
    if (lon > 95.0 && lat < 28.0) return false;

    return true;
  };

  const filteredHeatmapData = heatmapData.filter(point => {
    if (!isWithinIndia(point.lat, point.lon)) return false;
